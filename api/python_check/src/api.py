...
def analyze_single_file(filename: str, code: str, evaluation_config: Dict[str, bool]) -> Dict[str, Any]:
    ...
    if not filename.endswith('.py'):
        # If not a Python file, return a default "all pass" report
        return {
            "filename": filename,
            "pylint_score": 10.0,
            "flake8_score": 0,
...
async def analyze_batch_code_quality(
    ...
    evaluate_pylint: bool = Query(True),
    evaluate_flake8: bool = Query(True),
    evaluate_mypy: bool = Query(True),
    evaluate_complexity: bool = Query(True),
    evaluate_secrets: bool = Query(True)
) -> Dict[str, Any]:
    """Analyze multiple uploaded files and return batch results."""
    
    evaluation_config = {
        'evaluate_pylint': evaluate_pylint,
        'evaluate_flake8': evaluate_flake8,
        'evaluate_mypy': evaluate_mypy,
        'evaluate_complexity': evaluate_complexity,
        'evaluate_secrets': evaluate_secrets
    }
    ...
    for uploaded_file in files:
        ...
        filename = uploaded_file.filename if uploaded_file.filename else "unknown_file.py"
        
        # Check if file is a Python file
        if not filename.endswith('.py'):
            # Non-Python file: mark all validations as passed
            result = {
                "filename": filename,
                "pylint_score": 10.0,
                "flake8_score": 0,
        ...
        # Python file: try to decode and analyze
        try:
            code = content.decode('utf-8')
        except UnicodeDecodeError:
            # Binary file with .py extension: mark all validations as passed
            result = {
                "filename": filename,
                "pylint_score": 10.0,
                "flake8_score": 0,
...
@app.post("/analyze-single-file-upload", response_model=CodeQualityReport)
async def analyze_single_file_upload(
    ...
    """Analyze a single uploaded file and return quality report."""
    
    filename = file.filename if file.filename else ""
    if not filename.endswith('.py'):
        # Return a default "all pass" report for non-Python files
    ...
    # Create request object for reuse of existing logic
    request = CodeRequest(
        code=code,
        evaluate_pylint=evaluate_pylint,
        evaluate_flake8=evaluate_flake8,
    ...
    return await analyze_full_code_quality(request)

@app.post("/analyze-batch-code-quality-body", response_model=Dict[str, Any])
async def analyze_batch_code_quality_body(
    request: BatchCodeRequest
) -> Dict[str, Any]:
    """Analyze multiple files from request body and return batch results."""
